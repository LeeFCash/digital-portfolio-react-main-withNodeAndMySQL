...
export function Skills() {
	const [data, setData] = useState([]);
        useEffect(()=> {
                ...
                .then(res => res.json())
		.then((data) => {
        		const blogData = data[0]; // Assuming the JSON structure matches your example
        		const parsedBlogs = blogData.blogs.split(",").map((blog) => {
          		const [id, link] = blog.split(": ");
         	};
		...
		{data.map ((blog, index) => (
			<div
				...
				className="cV"
			>
				<img className="vIMG" src="defaultImagePath.jpg" alt="404" />
				<p className="vText">{`Blog#${data.blog_id}`}</p>
			</div>
		))}
      	</section>
